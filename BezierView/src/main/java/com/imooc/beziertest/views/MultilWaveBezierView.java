...
import com.imooc.beziertest.R;

/**
 * 新人匹配动画界面
 */
public class MultilWaveBezierView extends View implements View.OnClickListener {
	private Path mPath;
	...
	public MultilWaveBezierView(Context context) {
		super(context);
	}

	public MultilWaveBezierView(Context context, AttributeSet attrs) {
		super(context, attrs);
		...
		mPaintBezier.setColor(getResources().getColor(R.color.colorEnd));
		mPaintBezier.setStrokeWidth(1);
		mPaintBezier.setStyle(Paint.Style.STROKE);

		mPaintWhite = new Paint(Paint.ANTI_ALIAS_FLAG);
		mPaintWhite.setColor(Color.LTGRAY);
		mPaintWhite.setStrokeWidth(1);
		mPaintWhite.setStyle(Paint.Style.STROKE);
		mWaveLength = width / 2;
	}

	public MultilWaveBezierView(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	protected void onDraw(final Canvas canvas) {
		...
		mWhitePath2.reset();
		//Y坐标每次绘制时减去偏移量，即波浪升高
		mPath.moveTo((float) ((-mWaveLength + mOffsetX) * 6), mCenterY);
		mWhitePath.moveTo((float) ((-mWaveLength + mOffsetX) * 2), mCenterY);
		mWhitePath2.moveTo((float) ((-mWaveLength + mOffsetX) * 4), mCenterY);

		//每次循环绘制两个二阶贝塞尔曲线形成一个完整波形（含有一个上拱圆，一个下拱圆）
		for (int i = 0; i < mWaveCount; i++) {
			mPath.rQuadTo(mWaveLength / 2, -80, mWaveLength, 0);
			mPath.rQuadTo(mWaveLength / 2, +80, mWaveLength, 0);
		}

		for (int i = 0; i < mWaveCount; i++) {
			mWhitePath.rQuadTo(mWaveLength / 2, -120, mWaveLength, 0);
			mWhitePath.rQuadTo(mWaveLength / 2, +120, mWaveLength, 0);
		}

		for (int i = 0; i < mWaveCount; i++) {
			mWhitePath2.rQuadTo(mWaveLength / 2, -160, mWaveLength, 0);
			mWhitePath2.rQuadTo(mWaveLength / 2, +160, mWaveLength, 0);
		}
